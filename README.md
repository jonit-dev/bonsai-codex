...
| `docs/llama-codex.md` | the proxy's own documentation: wire translation, every `LLAMA_CODEX_*` variable, wrapper flags |
| `models-testing-sandbox/`, `local-model-*.md` | upstream material about testing local models generally, kept as it is |

The proxy's own documentation — the wire translation, every `LLAMA_CODEX_*` variable, the
wrapper's flags — is in [docs/llama-codex.md](docs/llama-codex.md). Everything below is about
running the model on this machine; the numbers are real runs, not estimates.
